...
class NoteViewSet(
    ...
    """
    Viewset handle create, list, retrive, update and delete notes
    """
    ...
    def voice(self, request, pk):
        """
        Decode Wave file to base64 and create new VoiceMemo object with it
        """
        ...
        with transaction.atomic():
            vm = VoiceMemo.objects.create(
                note=note, file=f"data:audio/wav;base64,{base64_message}"
            )
            vm.save()
        return Response(status=status.HTTP_201_CREATED)


class VoiceMemoViewSet(mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    """
    Viewset handle retrive voice memos
    """
    ...
    def get_queryset(self):
        return VoiceMemo.objects.filter(note__owner=self.request.user)
